...
- **고/스톱 (무제한 · 밀치기)**: 목표 도달 시 스톱(정산) 또는 고. n고 문턱 = 기본 목표 ×(1+0.5n), 성공 시 보너스 n(n+3)냥. 고 선언 시 현재 점수가 이미 넘은 문턱은 **전부 소급 인정(밀치기)** — 큰 한 방이면 1고→3고처럼 여러 단계를 한 번에 점프하고, 선언 목표는 "아직 못 넘은 첫 문턱"이라 항상 현재 점수보다 높다(공짜 연쇄 고 없음). 실패하면 **고박 = 런 종료**.
