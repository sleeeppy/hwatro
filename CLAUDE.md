...
- 고 목표는 항상 `ceil(baseTarget × goMult(goLevel))` — 현재 target에 거듭 곱하지 말 것.
...
- 카드 칩 적용 순서: 기본칩 → 이달의 패 ×2 → 밤낮 +3 → 박 디버프 0 (순서 바꾸면 밸런스 붕괴).

## 디자인 방향 (사용자 확정 사항)

- **고는 무제한** — n고 = 목표 ×(1+0.5n), 보너스 n(n+3)냥. 캡 되돌리지 말 것.
- **밸런스는 빡빡하게** — 난이도 조정 시 하향보다 상향 우선. 목표 커브(320→7,400)는 시뮬레이션 기준: 무조커 3월 벽 ~40%, 봇 승률 ~0%.
